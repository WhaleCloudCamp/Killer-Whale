            ...
            <Accordion >
                <Accordion.Panel header={<div className={styles.myaccordion}>{type}</div>}  >
                    ...
                    {Object.keys(value).map(items => {
                        ...
                        if ((items.includes("Color") || items.includes("color")) && (['white', 'red', 'blue', 'gray'].includes(value[items]) || value[items].includes("rgb") || /^#([0-9a-fA-F]{6}|[0-9a-fA-F]{3})$/.test(value[items]))) {
                            return (
